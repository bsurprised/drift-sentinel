...
import type { DriftConfig, DriftReport, DriftIssue, DocReference, DocSource, Verifier } from './types.js';
...
export async function runPipeline(root: string, config: DriftConfig): Promise<DriftReport> {
  ...
  function kindAllowed(kind: DriftKind): boolean {
  ...
  // 8. Run verifiers on each reference — sequential for loop, no parallelism
